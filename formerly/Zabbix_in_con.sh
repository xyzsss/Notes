...
yum install -y telnet nmap traceroute man ntp wget mlocate unzip dos2unix perl make gcc  vim

chkconfig ntpd on
/etc/init.d/ntpd start

echo "date.timezone = Asia/Chongqing" >> /etc/php.ini
...
>grant all on zabbix.* to 'zabbix'@'localhost' identified by 'xxxxx';       
>flush privileges;
>exit;
